...
def get_default_weights() -> Dict[str, float]:
    """Returns the default compliance scoring weights for pan-India tender evaluation."""
    return {
...
def recalculate_compliance_score(summary_json: Dict, weights: Dict[str, float]) -> Dict:
    ...
    Args:
        summary_json: The DPR summary JSON containing evaluationCriteria
    ...
    compliance = summary_json.get("evaluationCriteria")
    if not compliance:
        print("⚠ No compliance scoring data found in summary_json")
        return summary_json
    
    breakdown = compliance.get("criteriaBreakdown")
    if not breakdown:
        print("⚠ No criteria breakdown found in compliance data")
        return summary_json
    ...
    # ALSO update the top-level overallScore (used in DPR list display)
    summary_json["overallScore"] = round(new_score, 2)
...
def recalculate_project_dprs(project_id: int, weights: Dict[str, float], db_path: str = "data/dpr.db") -> Tuple[int, List[int]]:
    ...
    cursor.execute("""
        ...
        FROM dprs 
        WHERE project_id = %s AND summary_json IS NOT NULL
    """, (project_id,))
